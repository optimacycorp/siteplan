...
export function QuickMapCanvas() {
  ...
  const { drawings, addPoint } = useDrawingStore();

  const layers = useMemo(() => buildMapLayers({ selectedParcel, neighbors, drawings, layerVisibility }), [selectedParcel, neighbors, drawings, layerVisibility]);
  ...
  useEffect(() => {
    ...
    map.on("click", (event) => addPoint({ lng: event.lngLat.lng, lat: event.lngLat.lat }));
  ...
  useEffect(() => {
    ...
    if (bounds) map.fitBounds(new LngLatBounds(bounds[0], bounds[1]), { padding: 80, maxZoom: 19, duration: 350 });
